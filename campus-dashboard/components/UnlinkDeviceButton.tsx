...
export default function UnlinkDeviceButton({ childHashes }: Props) {
  ...
  const handleUnlink = async () => {
    ...
    const confirmed = window.confirm(
      childHashes.length === 1
        ? 'Unlink this student device? The dashboard will stop receiving updates and its history will be deleted. You can re-pair anytime with a new code.'
        : `Unlink all ${childHashes.length} linked student devices? The dashboard will stop receiving updates from all of them and their histories will be deleted.`,
    );
    if (!confirmed) return;
