...
-- Reused verbatim from SendWiseForensic/supabase/migrations/20260831110905_audit_log.sql
-- with forensic-only fields stripped (no warrant/evidence/jurisdiction refs).
-- Hash chain semantics are preserved exactly.
--
...
COMMENT ON COLUMN audit_log.hash IS
  'Integrity: sha256(prev_hash || canonical row payload). Chain break detects tampering.';
